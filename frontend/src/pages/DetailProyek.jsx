...
const handleUpdateProject = async (e) => {
  ...
  try {
    ...
    // JANGAN hanya setProyek(res.data.data) jika response backend tidak lengkap.
    // Cara paling aman: Gabungkan data lama dengan data baru
    setProyek((prev) => ({
      ...prev,
      ...res.data.data // Data terbaru dari backend
    }));

    setIsEditModalOpen(false);
    alert("Data proyek & anggaran berhasil diperbarui!");
  } catch (err) {
    console.error(err);
  ...
  if (loading || !proyek) return <div className="p-10 text-center animate-pulse text-gray-500 uppercase font-black">Memuat Detail...</div>;
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-slate-900 p-10 text-white relative">
            <button
                ...
                text-[10px] font-black uppercase px-4 py-1.5 rounded-full inline-flex items-center gap-2 hover:opacity-90 transition`}
            >
                {proyek.status} <span className="text-[10px]">▼</span>
            </button>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Update Project Information">
        <form onSubmit={handleUpdateProject} className="space-y-5">
          <div>
            ...
            <input 
              className="w-full bg-slate-50 p-4 rounded-2xl outline-none focus:ring-2 ring-blue-500/20 border-transparent border-2 focus:border-blue-500 transition"
              value={editData.nama} 
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select 
                className="w-full bg-slate-50 p-4 rounded-2xl outline-none appearance-none"
